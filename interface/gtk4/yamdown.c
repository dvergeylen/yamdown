...
/* helpers when interface buttons are clicked */
...
/* helpers when startup / open / activate events are fired */
static void on_open(GApplication *application, GFile **files,
...
static void on_activate(GApplication *application) {
  GtkApplication *app = GTK_APPLICATION(application);
  GtkWidget *win;
  GtkNotebook *notebook;
  GtkWidget *notebook_widget;
...
static void on_startup(GApplication *application) {
  ...
  /* Loading interface from gresource compiled file.
   * Recompile resource via:
   * glib-compile-resources yamdown.gresource.xml --target=ui-resources.h --generate-source
   */
  build = gtk_builder_new_from_resource("/com/github/dvergeylen/yamdown/gtk4/yamdown.ui");
